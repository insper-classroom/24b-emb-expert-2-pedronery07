...
*/
int32_t compensate_temp(int32_t adc_T, struct infos *inf) {
    int32_t var1, var2, T;
    var1 = ((((adc_T >> 3) - ((int32_t) inf->dig_T1 << 1))) * ((int32_t) inf->dig_T2)) >> 11;
    var2 = (((((adc_T >> 4) - ((int32_t) inf->dig_T1)) * ((adc_T >> 4) - ((int32_t) inf->dig_T1))) >> 12) * ((int32_t) inf->dig_T3))
            >> 14;

    inf->t_fine = var1 + var2;
    T = (inf->t_fine * 5 + 128) >> 8;
    return T;
}

uint32_t compensate_pressure(int32_t adc_P, struct infos *inf) {
    int32_t var1, var2;
    uint32_t p;
    var1 = (((int32_t) inf->t_fine) >> 1) - (int32_t) 64000;
    var2 = (((var1 >> 2) * (var1 >> 2)) >> 11) * ((int32_t) inf->dig_P6);
    var2 = var2 + ((var1 * ((int32_t) inf->dig_P5)) << 1);
    var2 = (var2 >> 2) + (((int32_t) inf->dig_P4) << 16);
    var1 = (((inf->dig_P3 * (((var1 >> 2) * (var1 >> 2)) >> 13)) >> 3) + ((((int32_t) inf->dig_P2) * var1) >> 1)) >> 18;
    var1 = ((((32768 + var1)) * ((int32_t) inf->dig_P1)) >> 15);
    if (var1 == 0)
        return 0;
    ...
    var1 = (((int32_t) inf->dig_P9) * ((int32_t) (((p >> 3) * (p >> 3)) >> 13))) >> 12;
    var2 = (((int32_t) (p >> 2)) * ((int32_t) inf->dig_P8)) >> 13;
    p = (uint32_t) ((int32_t) p + ((var1 + var2 + inf->dig_P7) >> 4));

    return p;
}

uint32_t compensate_humidity(int32_t adc_H, struct infos *inf) {
    int32_t v_x1_u32r;
    v_x1_u32r = (inf->t_fine - ((int32_t) 76800));
    v_x1_u32r = (((((adc_H << 14) - (((int32_t) inf->dig_H4) << 20) - (((int32_t) inf->dig_H5) * v_x1_u32r)) +
                   ((int32_t) 16384)) >> 15) * (((((((v_x1_u32r * ((int32_t) inf->dig_H6)) >> 10) * (((v_x1_u32r *
                                                                                                  ((int32_t) inf->dig_H3))
            >> 11) + ((int32_t) 32768))) >> 10) + ((int32_t) 2097152)) *
                                                 ((int32_t) inf->dig_H2) + 8192) >> 14));
    v_x1_u32r = (v_x1_u32r - (((((v_x1_u32r >> 15) * (v_x1_u32r >> 15)) >> 7) * ((int32_t) inf->dig_H1)) >> 4));
...
void read_compensation_parameters(struct infos *inf) {
    uint8_t buffer[26];

    read_registers(0x88, buffer, 26);

    inf->dig_T1 = buffer[0] | (buffer[1] << 8);
    inf->dig_T2 = buffer[2] | (buffer[3] << 8);
    inf->dig_T3 = buffer[4] | (buffer[5] << 8);

    inf->dig_P1 = buffer[6] | (buffer[7] << 8);
    inf->dig_P2 = buffer[8] | (buffer[9] << 8);
    inf->dig_P3 = buffer[10] | (buffer[11] << 8);
    inf->dig_P4 = buffer[12] | (buffer[13] << 8);
    inf->dig_P5 = buffer[14] | (buffer[15] << 8);
    inf->dig_P6 = buffer[16] | (buffer[17] << 8);
    inf->dig_P7 = buffer[18] | (buffer[19] << 8);
    inf->dig_P8 = buffer[20] | (buffer[21] << 8);
    inf->dig_P9 = buffer[22] | (buffer[23] << 8);

    inf->dig_H1 = buffer[25]; // 0xA1

    read_registers(0xE1, buffer, 8);

    inf->dig_H2 = buffer[0] | (buffer[1] << 8); // 0xE1 | 0xE2
    inf->dig_H3 = (int8_t) buffer[2]; // 0xE3
    inf->dig_H4 = buffer[3] << 4 | (buffer[4] & 0xf); // 0xE4 | 0xE5[3:0]
    inf->dig_H5 = (buffer[4] >> 4) | (buffer[5] << 4); // 0xE5[7:4] | 0xE6
    inf->dig_H6 = (int8_t) buffer[6]; // 0xE7
}
...
int main() {
    ...
    read_compensation_parameters(&informacoes);
    ...
    while (1) {
        ...
        temperature = compensate_temp(temperature, &informacoes);
        pressure = compensate_pressure(pressure, &informacoes);
        humidity = compensate_humidity(humidity, &informacoes);
